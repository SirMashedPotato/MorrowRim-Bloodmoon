...
namespace MorrowRim_Bloodmoon
{
	class IncidentWorker_MakeGameConditionRogue : IncidentWorker
	{
		...
		{
			if (!base.CanFireNowSub(parms) || !Bloodmoon_ModSettings.EnableBloodmoonForAll || FiredTooRecently_Setting(parms.target) || FiredTooEarly_Setting() || !Utility.HoundsFactionFound(def.label))
			{
				return false;
			}
		...
		public bool FiredTooRecently_Setting(IIncidentTarget target)
		{
			...
			int num;
			if (lastFireTicks.TryGetValue(this.def, out num) && (float)(ticksGame - num) / 60000f < Bloodmoon_ModSettings.IncidentIntervalRogue)
			{
				return true;
			}
			...
			if (refireCheckIncidents != null)
			{
				...
				{
					if (lastFireTicks.TryGetValue(refireCheckIncidents[i], out num) && (float)(ticksGame - num) / 60000f < Bloodmoon_ModSettings.IncidentIntervalRogue)
					{
						return true;
					}
				}
		...
		{
			return GenDate.DaysPassedSinceSettle < Bloodmoon_ModSettings.IncidentMinimumDaysRogue;
		}
